"""An offline password manager/vault. Stores passwords with AES encryption with accompanying username and website. 

To Start, set up a master password for accessing stored passwords. 

Use Add Passwords to add passwords.

Use View Passwords to view and delete saved passwords.

Double Click on any entry and enter master password to view passwords."""
...
def verify_master_password():
    ...
    if master_password:
        ...
        if master_key and verify_password(stored_hash.decode(), master_password):
            return master_key, master_password
        else:
...
class AddPasswordFrame(tk.Frame):
    ...
    def save_password(self):
        ...
        if website and username and password:
            master_key, master_password = verify_master_password()
            if master_key:
                ...
                conn = sqlite3.connect(DATABASE_FILE)
                c = conn.cursor()
                c.execute("INSERT INTO passwords (id, website, username, password, key) VALUES (?, ?, ?, ?, ?)",
                        (next_id, website, username, encrypted_password, encrypted_key))
                conn.commit()
                conn.close()
...
class ViewPasswordsFrame(tk.Frame):
    ...
    def delete_password(self):
        ...
        if confirm:
            master_key, _ = verify_master_password()
            if master_key:
                conn = sqlite3.connect(DATABASE_FILE)
                c = conn.cursor()
    ...
    def reconfigure_ids(self):
        ...
        c.execute("SELECT * FROM passwords ORDER BY id")
        rows = c.fetchall()
        ...
        # insert data back with reconfigured IDs
